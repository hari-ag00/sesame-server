...
class ModelRunner:
    ...
    async def initialize(self):
        ...
        # Critical: Set up model caches for initial batch size
        print("Setting up model caches...")
        try:
            self.model.setup_caches(1)
        except Exception as e:
            print(f"Warning: Failed to setup caches during initialization: {e}")
            print("Will retry before generation.")
    ...
    async def _process_requests(self):
        ...
        while not self.batch_manager.shutdown_flag:
            try:
                # Get the next batch of requests - force at least one request if available
                batch = await self.batch_manager.get_next_batch(config.batch.max_batch_size)
                ...
                if batch:
                    ...
                    # Wait for new requests with a short timeout
                    print("No requests to process, waiting...")
                    try:
                        # Only wait for a short time to avoid being stuck
                        await asyncio.wait_for(self.batch_manager.wait_for_requests(), timeout=1.0)
                    except asyncio.TimeoutError:
                        # No requests arrived, continue the loop
                        pass
